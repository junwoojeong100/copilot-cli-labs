...
- 바이브 코딩으로 규칙에 맞는 코드를 생성·리뷰하고, 가드레일(`AGENTS.md`)로 안전하게 커밋/PR 할 수 있습니다.
...
export GITHUB_PERSONAL_ACCESS_TOKEN=ghp_xxx   # 최소 권한의 fine-grained PAT 권장
az login
...
> 있으므로**(예: `/sandbox`·`/every`·`/after` 등 일부는 버전·실험적 기능에 따라 노출이 달라질 수 있음),
